...
function profilesRouterFactory (
    ...
    router.get('/profiles', function (req, res) {
        if (req.param('macs')) {
            // Arista zerotouch ends up escaping the ampersand in the
            // request (?macs=<mac>\&profiles), so remove it.
            var macs = req.param('macs');
            if (typeof macs === 'string') {
                macs = req.param('macs').replace(/\\/, '');
            } else if (lodash.isArray(macs)) {
                macs = lodash.map(macs, function(mac) {
                    return mac.replace(/\\/, '');
                });
            }
            var macAddresses = lodash.flatten([macs]);
