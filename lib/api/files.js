...
function filesRouterFactory (Logger, configuration, FileService, assert) {
    ...
    router.put('/files/:filename', function(req, res) {
        ...
        .then(function(transformStreamObj) {

            assert.object(transformStreamObj, 'transform Stream object');

            transformStreamObj.stream.on('error', function(err) {
                logger.error("Failure serving file request", { error: err });
                res.status(500).json({
                    error: "Failed to serve file request."
                });
            ...
            transformStreamObj.stream.on('end', function() {
                res.status(201);
            });

            req.pipe(transformStreamObj.stream);
            res.send(transformStreamObj.id);
        })
        .catch(function() {
            res.status(500).json({
                error: "Failed to serve file request."
