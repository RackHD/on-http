    ...
    .then(function(nodes) {
        ...
        if(parts[1]) {
            _.forEach(nodes, function(node) {
                assert.ok(_.isArray(node.obmSettings), 'invalid resource');
                assert.ok(node.obmSettings.length > parts[1], 'invalid resource');
            });
        }
...
var getLocalEthernetInterface = controller(function(req, res) {
    ...
    if(!net) {
        return new Errors.NotFoundError('invalid resource');
    }
