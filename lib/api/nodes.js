...
function nodesRouterFactory (
    ...
    router.get('/nodes/:identifier', presenter.middleware(function (req) {
        return waterline.nodes.findByIdentifier(req.params.identifier);
    }));
    ...
    router.get('/nodes/:identifier/catalogs/:source', presenter.middleware(function (req, res) {
        return waterline.nodes.findByIdentifier(req.params.identifier)
            ...
            .catch(function(error) {
                logger.warning("Error searching for catalog " + req.params.source +
                    " on node id " + req.params.identifier, { error: error });
                res.status(404);
