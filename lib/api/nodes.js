...
function nodesRouterFactory (
     ...
     * @apiName nodes-post
     * @apiParam {String|[String]} identifiers Mac addresses and unique aliases to
    ...
    router.get('/nodes/:identifier/workflows', presenter.middleware(function (req) {
        return waterline.workflows.find(
            _.merge(
                req.query || {},
                { where: { node: req.param('identifier'), type: 'parentWorkflow' }}
            )
        ).populate('workflows');
    }));
    ...
    router.post('/nodes/:identifier/workflows', presenter.middleware(function(req) {
        return waterline.nodes.findByIdentifier(req.param('identifier')).then(function (node) {
            if (node) {
                return Q.resolve(workflowService.createWorkflow(
                    node.id,
                    req.param('name'),
                    req.param('options')
                )).then(function(workflow) {
                    return new Q.Promise(function (resolve, reject) {
                        // TODO: leverage another mechanism to wait for the workflow to be persisted
                        setImmediate(function () {
                            waterline.workflows.find(
                                { where: { instance: workflow.instance }}
                            ).then(resolve, reject);
                        });
                    });
                });
    ...
    router.get('/nodes/:identifier/workflows/active', presenter.middleware(function (req) {
        return waterline.nodes.findByIdentifier(req.param('identifier')).then(function (node) {
            if (node) {
                return workflowService.getWorkflow(node.id);
            }
            // TODO: should there be a not found block here
            // that renders 404 or does it do that implicitly.
        });
    }));
