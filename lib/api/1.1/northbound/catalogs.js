...
di.annotate(catalogsRouterFactory, new di.Inject(
        ...
        'Http.Services.Api.Catalogs'
    )
);

function catalogsRouterFactory (waterline, rest, catalogApiService) {
    ...
    router.get('/catalogs', rest(function (req) {
        return catalogApiService.getCatalog(req.query);
    }));
    ...
    router.get('/catalogs/:identifier', rest(function (req) {
        return catalogApiService.getCatalogById(req.params.identifier);
    }));
