...
function skusRouterFactory (
    ...
    router.skuPackHandler = function(req,res,skuid) {
        var name = uuid('v4');
        var tmpDir = tmp();
        var skuName = '';
        try {
            req.pipe(zlib.createGunzip())
                ...
                .on('error', function() {
                    res.status(500).json({ error: 'Failed to serve file request' });
                });
        } catch(err) {
            res.status(500).json({ error: 'Failed to serve file request' });
        }
    ...
    router.post('/skus/pack', function(req, res) {
        return router.skuPackHandler(req,res);
    });
    ...
    router.put('/skus/:identifier/pack', function(req, res) {
        return waterline.skus.needByIdentifier(req.params.identifier)
            .then(function() {
                return router.skuPackHandler(req,res,req.params.identifier);
            });
    });
