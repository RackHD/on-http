...
function templatesRouterFactory (
    ...
    router.get('/templates/library/:identifier', lookupMiddleware, function (req, res) {
        presenter(req, res)
            .renderPlain(templateService.get(req.param('identifier')));
    });

    router.put('/templates/library/:identifier', lookupMiddleware, parser.raw(),
               function (req, res) {
        templateService.put(req.param('identifier'), req.body).then(function () {
            res.status(200).send();
