        ...
        .then(function(task){
            if (!_.isEmpty(task) && _.has(task, 'graphId')){
                ...
                .then(function(graphObject){
                    if (_.has(progress, "totalSteps") &&
                        _.has(progress, "currentStep")) {
                        progress.progressRate = progress.currentStep +
                            '/' + progress.totalSteps;
                        progress = _.omit(progress, ['totalSteps', 'currentStep']);
                    } else if(!_.has(progress, "percentage")) {
                        logger.error('notification API fails', {
                            taskId: taskId,
                            progress: progress,
                            error: "Progress data has no valid progress measurement data"
                        });
