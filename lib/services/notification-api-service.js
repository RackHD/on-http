        ...
        .then(function(task){
            if (!_.isEmpty(task) && _.has(task, 'graphId')){
                ...
                .then(function(graphObject){
                    ...
                    if (_.has(data, "progress.totalSteps") &&
                        _.has(data, "progress.currentStep") &&
                        !_.has(data, "progress.percentage") &&
                        !_.has(data, "progress.progressRate")) {
                        //var percent = new Number(100*_.parseInt(data.progress.currentStep)/
                        //               _.parseInt(data.progress.totalSteps));
                        data.progress.progressRate = data.progress.currentStep +
                            '/' + data.progress.totalSteps;
                        data.progress = _.omit(data.progress, ['totalSteps', 'currentStep']);
                    }
                    progressData = {
