...
function swaggerFactory(
    ...
    function addLinksHeader(req, res, count) {
        ...
        if ( skip && skip < top) {
            res.links.first = util.format('%s?$skip=0&$top=%d', uriBase, skip);
        } else {
            res.links.first = util.format('%s?$skip=0&$top=%d', uriBase, top);
        }
        ...
        if (skip) {
            if (skip < top) {
                res.links.prev = util.format('%s?$skip=0&$top=%d', uriBase, skip);
            } else {
                res.links.prev = util.format(
                    '%s?$skip=%d&$top=%d',
                    uriBase,
                    skip - top,
                    top
