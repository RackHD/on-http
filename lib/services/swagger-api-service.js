...
function swaggerFactory(
    ...
    function _processRedfishCatalog(req, res, next){
        // if (!req.swagger.params.identifier) {
        //     return Promise.resolve(undefined);
        // }
        var identifier = req.swagger.params.identifier.value;
        //var catalogSource = req.url.split(['-'])[0];
        var nodeId = identifier.split(['-'])[1];
        var origIdentifier = identifier.split(['-'])[0];
        var regex = new RegExp('-' + nodeId, 'g');
        var catalogSource = req.url.replace(regex, '');
        var checkURL = ["AirHandlingUnit","CoolingTower", "CRAH", "CARC", "Chiller", "Generator", "RackPDU", "PDU", "Rectifier","Switchgear", "Transformer", "UPS","VFD", "TransferSwitch" ];
        var extractLastURLString = catalogSource.split(/[\/ ]+/).pop();
	    var splitNameNumber = extractLastURLString.split(/(\d+)/);
        var componentName = splitNameNumber[0];
        var componentNumber = splitNameNumber[1];
        if ( _.include(checkURL, componentName )){
            //var modifyCatalogSource = catalogSource.split(/[\/ ]+/);
            var newString = componentName+'/'+componentNumber;
            var re = new RegExp(extractLastURLString, "g");
            catalogSource = catalogSource.replace(re,componentNumber);
        }
            return nodeApi.getNodeCatalogSourceById(nodeId, catalogSource)
                .then(function (catalogData) {
                    if (catalogData) {
                        var catalogDataString = JSON.stringify(catalogData.data);
                        //var odataWithNode = catalogDataString.replace(/("@odata.id":".*?")/g, "$1" + nodeId + '"');
                        //var regex = new RegExp('"' + nodeId, 'g');
                        var regex = new RegExp('/' + origIdentifier, 'g');
                        //var quoteRemoved = odataWithNode.replace(regex, '-' + nodeId);
                        var odataWithNode = catalogDataString.replace(regex, '/' + identifier);
                        //var quoteRemoved = catalogDataString.replace(/System.Embedded.1/g, 'System.Embedded.1-' + nodeId);
                        var retData = JSON.parse(odataWithNode.replace(/@odata_/g, '@odata.'));
                        return retData;
                    } else {
                        return undefined;
                    }
    ...
    function swaggerController(options, callback) {
        ...
        return function(req, res, next) {
            ...
            var exclude = ["DCIM"];
            if (req.url.startsWith('/redfish') && req.swagger.params.identifier && !_.contains(req.url, 'default-')) {
                return _processRedfishCatalog(req, res, next)
                    .then(function () {
            ...
            else if (_.contains(req.url, exclude)  && req.swagger.params.identifier && req.url.match(/\d/g) && !_.contains(req.url, 'default-')){
                return _processRedfishCatalog(req, res, next)
                    .then(function () {
                        if (!res.headersSent && res.body) {
                            next();
                ...
                return Promise.try(function () {
                    ...
                    if (!res.headersSent && result) {
                        if (_.isArray(result)) {
                            ...
                            if (toSort) {
                                (toSort.type === 'string') ? res.body.sort(_sortByAny(toSort.sortBy, toSort.dsc, function (a) {
                                    return a.toUpperCase()
                                })) : res.body.sort(_sortByAny(toSort.sortBy, toSort.dsc, parseInt)); // jshint ignore:line
