...
function authServiceFactory(
    ...
    AuthService.prototype.loadFromConf = function (){
        ...
        this.secretOrKey = new Buffer(configuration.get('authTokenSecret',
                'RackHDRocks!'),
            this.encoder);

        assert.string(this.userFromConf, 'Invalid username in configure file');
        assert.string(this.hashFromConf, 'Invalid password hash in configure file');
        assert.string(this.saltFromConf, 'Invalid crypto salt in configure file');
        assert.string(this.secretOrKey, 'Invalid token secret in configure file');
