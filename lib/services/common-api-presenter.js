...
function CommonApiPresenterFactory(
    ...
    function presenter (req, res) {
        ...
        CommonApiPresenter.prototype.renderError = function (error, options, status) {
            ...
            logger.error("CommonApiPresenter Error: " + error.message, {
                error: error,
                path: this.req.path
            });
         ...
         */
        CommonApiPresenter.prototype.renderTemplate = function (template, options, status) {
            var self = this;
            ...
            Q.all(promises).spread(function (macaddress, contents) {
                var output;
                ...
                try {
                    ...
                    self._publish(macaddress, 'rendered.template', { name: template });
                } catch (err) {
                    self._publish(macaddress, 'rendered.error', {
                        error: err.message,
                        options: options,
                        type: 'template'
                    });
        ...
        CommonApiPresenter.prototype.renderProfile = function (profile, options, status) {
            ...
            // If the profile doesn't have an extension we need to patch it up.
            if (!profile.match(/\..*/)) {
                logger.debug('Profile missing extension.', {
                    profile: profile
                });

                var profiles = profileService.getAllSync();

                // Look for a profile named the same as the requested.
                profile = _.find(_.keys(profiles), function (target) {
                    if (target.startsWith(profile)) {
                        logger.debug('Updating profile extension.', {
                            previous: profile,
                            updated: target
                        });
            ...
            if (profile.endsWith('.ipxe')) {
                promises.push(profileService.get('boilerplate.ipxe'));
                promises.push(profileService.get('error.ipxe'));
            }
            ...
            Q.all(promises).spread(
                function (macaddress, contents, boilerPlate, errorPlate) {
                    ...
                    try {
                        ...
                        // Notify the workflow about the rendering.
                        self._publish(macaddress, 'rendered.profile', { name: profile });
                    } catch (err) {
        ...
        CommonApiPresenter.prototype._publish = function(macaddress, topic, data) {
            lookupService.macAddressToNodeId(macaddress).then(function (identifier) {
                ...
                logger.debug('Unable to publish presenter events to unknown node.', {
                    error: err,
                    macaddress: macaddress,
                    topic: topic,
                    data: data
