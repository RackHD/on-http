...
function CommonApiPresenterFactory(
    ...
    CommonApiPresenter.prototype._buildContext = function(graphContext, ignoreLookup) {
        ...
        var fileServerUri;
        if (_.has(config, 'fileServerAddress')) {
            fileServerUri = 'http://' + config.fileServerAddress;
            if (_.has(config, 'fileServerPort')) {
                fileServerUri = fileServerUri + ':' + config.fileServerPort;
            }
            if (_.has(config, 'fileServerPath')) {
                fileServerUri = fileServerUri + config.fileServerPath;
                fileServerUri = _.trimRight(fileServerUri, '/');
            }
        } else {
            fileServerUri = serverUri;
        }
