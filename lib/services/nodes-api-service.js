...
function nodeApiServiceFactory(
    ...
    NodeApiService.prototype.postNode = function(body) {
        ...
        .then(function(node) {
            if(node.type === Constants.NodeTypes.Switch && node.snmpSettings && node.autoDiscover) {
            ...
            else if(node.type === Constants.NodeTypes.Pdu && node.snmpSettings && node.autoDiscover) {
                return workflowApiService.createAndRunGraph(
                    {
            ...
            else if(node.type === Constants.NodeTypes.Mgmt && node.obmSettings && node.autoDiscover) {
                var configuration = {
