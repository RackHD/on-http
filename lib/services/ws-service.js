...
function webSocketServiceFactory(
    ...
    WebSocketService.prototype.broadcast = function (data, skipFunc) {
        ...
        this.webSocketServer.clients.forEach(wsClientBroadcast.bind(this));
        ...
        function wsClientBroadcast(wsConn) {
            ...
            try {
                ...
                logger.warn('Failed to send message to client: %s'.format(wsConn.id));
    ...
    WebSocketService.prototype.start = function (httpServer, params) {
        if (!httpServer) {
    ...
    WebSocketService.prototype._closeConnection = function (wsConn) {
        ...
        logger.info('Client: %s disconnected from WebSocketServer.'.format(wsConn.id));
    ...
    WebSocketService.prototype._forwardResourceMethod = function (method, msg, wsConn, rawMsg) {
        logger.info('Client: %s requested a resource call.'.format(wsConn.id),
            {rawMessage: rawMsg});
    ...
    WebSocketService.prototype._handleConnection = function (wsConn) {
        ...
        logger.info('Client: %s connected to WebSocketServer.'.format(wsConn.id));
    ...
    WebSocketService.prototype._handleMessage = function (wsConn, wsMsg) {
        logger.info('Client: %s messsage was received.'.format(wsConn.id),
            {rawMessage: wsMsg});
