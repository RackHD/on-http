...
function factory(assert, _, injector, Errors) {
            ...
            uuidFound
                ...
                .then(function(rdStream) {
                    rdStream.on('error', function(err) {
                        reject(new Errors.BadRequestError('Failed to serve file request: ' + err.message));
                    });
                    ...
                    reject(new Errors.NotFoundError('Failed to get stream reader: ' + err.message));
                })
                .catch(function(err) {
                    reject(new Errors.NotFoundError('Failed to serve file request: ' + err.message));
                });
        });
    };
    ...
    FileStreamer.prototype.getFileMd5 = function(filename) {
        var self = this;

        return new Promise(function(resolve, reject) {
            self.verify(filename)
                .then(function(metadata){
                    resolve(metadata[metadata.length - 1].md5);
                })
                .catch(function(err) {
                    reject(new Errors.NotFoundError('Failed to serve file request: ' + err.message));
                });
