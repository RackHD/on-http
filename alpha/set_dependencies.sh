...
if [ "${VERIFY_DEP}" == "true" ]; then
    COMMIT=$(cat $(ls ../manifest-artifactory/manifest*.json) | jq -r .onhttp.commit)
    ...
    ln -s $(pwd)/on-tasks $(pwd)/on-http/node_modules/on-tasks
    ln -s $(pwd)/on-core $(pwd)/on-tasks/node_modules/on-core
    popd
    # Map on-core
    pushd ../
    ln -s $(pwd)/on-core $(pwd)/on-http/node_modules/on-core
    popd
